...
class KytosAdapter(Adapter):
    """
    Create a reverse proxy to adapt Unicorn APIs in Kytos.
    """
    ...
    def get_as_path(self, query_set):
        ...
        if connexion.request.is_json:
            ...
            for query in query_set:
                ...
                """
                egress = self.get_egress_point(query)
    ...
    def get_resource(self, query_set):
        ...
        if connexion.request.is_json:
            ...
            for query_idx, query in enumerate(query_set):
                used_links = self.get_links_from_route(query)
    ...
    #        pprint(A)
            ...
            for idx in range(C.shape[0]):
                if np.count_nonzero(A[idx, :]) > 0:
                    C[idx] = self.get_availbw(all_links[idx])
    #        pprint(C)
    ...
    def get_egress_point(self, query):
